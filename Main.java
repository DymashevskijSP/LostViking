...
public class Main {
    ...
    static class EnemyShip {
        ...
        public EnemyShip(int size, double x, int y) {
            img = new ImageIcon("src/Dymashevskij_Game/game/enemyssh.png");
            this.size = size;
            this.size = 55;
            this.x = x;
            this.y = y;
    ...
    static class Meteor {
        ...
        public Meteor(double vx, double vy, int size, double x, int y) {
            img = new ImageIcon("src/Dymashevskij_Game/game/meteor.png");
            this.vx = vx;
            this.vy = vy;
            this.size = size;
            this.x = x;
    ...
    static class EnemyBullet extends Meteor {
        public EnemyBullet(double vx, double vy, int size, double x, int y) {
            ...
            this.img = new ImageIcon("src/Dymashevskij_Game/game/enemybullet.jpg");
            //this.size = 55;
        }
    }
    ...
    static class MyPanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            ImageIcon img = new ImageIcon("src/Dymashevskij_Game/game/spaceship.jpg");
            if (laseralive > 0) {
            ...
            for (int i = 0; i < enemies.size(); i++) {
                for (int j = 0; j < bullets.size(); j++) {
                    if (i < enemies.size() && j < bullets.size() && (Math.abs(bullets.get(j).x - enemies.get(i).x - enemies.get(i).size / 2 + bullets.get(j).size / 2) <= enemies.get(i).size / 2 + bullets.get(j).size / 2 &&
                            Math.abs(bullets.get(j).y - enemies.get(i).y) <= 30)) {
                        bullets.remove(j);
            ...
            for (int z = 0; z < enemies.size(); z++) {
                if (enemies.get(z).getClass() == Wall.class) {
                    if (Math.abs(enemies.get(z).y - y + 30) < 10 && enemies.get(z).x < x + 50 && enemies.get(z).x + enemies.get(z).size > x && y + 80 > enemies.get(z).y) {
                        vy = -vy + 2;
                        y += 5;
                    }
                    if (Math.abs(enemies.get(z).y - y + 30) < 10 && enemies.get(z).x < x + 50 && enemies.get(z).x + enemies.get(z).size > x && y + 20 < enemies.get(z).y) {
                        vy = -vy - 2;
                        y -= 5;
                    }
                }
            ...
            for (; i < meteors.size(); i++) {
                for (int j = 0; j < bullets.size(); j++) {
                    if (i < meteors.size() && j < bullets.size() && (Math.abs(bullets.get(j).x - meteors.get(i).x) < 30 &&
                            Math.abs(bullets.get(j).y - meteors.get(i).y) < 30) && !meteors.get(i).getClass().toString().equals(EnemyBullet.class.toString())) {
                        bullets.remove(j);
                        meteors.remove(i);
                        Random r = new Random();
                        if (r.nextInt(50) == 30) {
    ...
    public static void main(String[] args) {
        ...
        hard.addAdjustmentListener(new AdjustmentListener() {
            @Override
            public void adjustmentValueChanged(AdjustmentEvent e) {
                tutorial.setText("<html><p>Press space to activate laser</p><p>current level of difficulty: " + (hard.getValue() + 1) + "</p></html>");
            }
        });
        ...
        strtgame[0].addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
        ...
        Panel.addMouseListener(new MouseListener() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                if (!Panel.dd) {
                    Panel.bullets.add(new Bullet("src/Dymashevskij_Game/game/bullet.png", 0, -8, 30, (int) Panel.x, (int) Panel.y));
                } else {
                    Panel.bullets.add(new Bullet("src/Dymashevskij_Game/game/bullet.png", 0, -8, 30, (int) Panel.x - 25, (int) Panel.y));
                    Panel.bullets.add(new Bullet("src/Dymashevskij_Game/game/bullet.png", 0, -8, 30, (int) Panel.x + 5, (int) Panel.y));
